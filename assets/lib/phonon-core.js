...
(function (window, riot, phonon) {
  ...
  function callHiddenCallback(pageName) {
	if( pageName === null)//Min patched 8/20/19
		return;
    if (riotEnabled) {
  ...
  function _changePage(pageName, pageParams) {
    ...
    if (pageObject) {
      ...
      if (typeof pageParams !== 'undefined') {
        ...
        callClose(currentPage, pageObject.name, hash);//Min changed 9/13/19
  ...
  function onRoute(virtualHash, postData) {
    var hash;
    if(typeof virtualHash == "undefined"){//for program just started
    	hash = '';
    }else
    	hash = typeof virtualHash === 'string' ? virtualHash : window.location.href.split('#')[1] || '';
    var pageName;
